...
for r=1:2*N
    ...
    x_rows=x_rows+EigValues_vec(r)*A_r*col1*exp(EigValues_vec(r)*t_row)+A_r*col2*exp(EigValues_vec(r)*t_row);
    
%For Display only
if imag(EigValues_vec(r))~=0 && mod(r,2)~=0
    disp('Free Response parameters');
    r
    2*(real(EigValues_vec(r))*real(A_r)-abs(imag(EigValues_vec(r)))*imag(A_r))*col1+2*real(A_r)*col2
    -2*(abs(imag(EigValues_vec(r)))*real(A_r)+real(EigValues_vec(r))*imag(A_r))*col1-2*imag(A_r)*col2
end
