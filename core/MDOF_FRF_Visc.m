function [H_cols,H_cols_SDOF_pages]=MDOF_FRF_Visc(s_q_vec,EigVectors_Normalized,w_col,m_vec,n_vec)

[P,Q]=size(EigVectors_Normalized);
if Q~=2*P,error('EigVectors_Normalized should be P x 2P matrix.'),end
N_w=size(w_col,1);
...
for q=1:Q
    ...
    p=ceil(q/2);
    H_cols_SDOF_pages(:,:,p)=H_cols_SDOF_pages(:,:,p)+A_q_row./(1i*w_col-s_q_vec(q));
end
