function [w_r_col,zeta_r_col,w_d_r_col]=pole2modal_visc(s_pairs_col)
if ~iscolumn(s_pairs_col),error('s_pairs_col must be a column vector'),end

N=length(s_pairs_col)/2;
...
w_r_col(r_underDamped_vec)=abs(s_pairs_col(q_underDamped_vec));
zeta_r_col(r_underDamped_vec)=-real(s_pairs_col(q_underDamped_vec))./w_r_col(r_underDamped_vec);

zeta_r_col(r_overDamped_vec)=sqrt(1./(1-((s_pairs_col(q_overDamped_vec)-s_pairs_col(q_overDamped_vec+1))./(s_pairs_col(q_overDamped_vec)+s_pairs_col(q_overDamped_vec+1))).^2));
w_r_col(r_overDamped_vec)=-(s_pairs_col(q_overDamped_vec)+s_pairs_col(q_overDamped_vec+1))/2./zeta_r_col(r_overDamped_vec);
