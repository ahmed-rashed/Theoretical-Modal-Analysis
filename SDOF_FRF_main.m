...
%% Viscous SDOF
    %Single curve
    SDOF_PlotFRF(r,@SDOF_FRF_Visc_mul_k,0.15,'','',{'H_{u}','k'},30,[],[],[]);
    ...
    SDOF_PlotFRF(r,@SDOF_FRF_Visc_mul_k,zeta_vec,'','',{'H_{u}','k'},DispMagLines,r_peaks,abs(H_mag_peaks),[]);
    ...
    SDOF_PlotFRF(r,@(r,zeta) 1i*r.*SDOF_FRF_Visc_mul_k(r,zeta),zeta_vec,'','',{'H_v','k/\omega_{\textrm{n}}'});
    ...
    SDOF_PlotFRF(r,@(r,zeta) -r.*r.*SDOF_FRF_Visc_mul_k(r,zeta),zeta_vec,'','',{'H_a','k/\omega_{\textrm{n}}^2'});
...
SDOF_PlotFRF(r,@(r,zeta) (2*1i*zeta*r+1)./(1-r.*r+2*1i*zeta*r),zeta_vec,'','','',DispMagLines,r_peaks,abs(H_mag_peaks),sqrt(2));
...
SDOF_PlotFRF(r,@(r,zeta) r.*r./(1-r.*r+2*1i*zeta*r),zeta_vec,'','','H_{u}',DispMagLines,r_peaks,abs(H_mag_peaks),[]);
...
%SDOF_PlotFRF(r,@(r,zeta) r/1i./(1-r.*r+2*1i*zeta*r),zeta_vec,'','','H_{\textrm{Vib}}',DispMagLines,[],[],[],false);
SDOF_PlotFRF(r,@(r,zeta) -1./(1-r.*r+2*1i*zeta*r),zeta_vec,'','',{'H_{\textrm{Acc}}','\omega^2_{\textrm{n}}'},DispMagLines,[],[],[]);
...
%% Hysteretic SDOF
SDOF_PlotFRF(r,@SDOF_FRF_Hyst_mul_k,zeta_vec,'','\eta',{'H_{u}','k'},DispMagLines,[],[],[]);
...
SDOF_PlotFRF(r,@(r,eta) 1i*r.*SDOF_FRF_Hyst_mul_k(r,eta),zeta_vec,'','\eta',{'H_v','k/\omega_{\textrm{n}}'});
...
SDOF_PlotFRF(r,@(r,eta) -r.*r.*SDOF_FRF_Hyst_mul_k(r,eta),zeta_vec,'','\eta',{'H_a','k/\omega_{\textrm{n}}^2'});
