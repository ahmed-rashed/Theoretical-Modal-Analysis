...
%Viscous SDOF
    %Single curve
    SDOF_PlotFRF(r,@SDOF_FRF_Visc_mul_k,0.15,'$r$','\zeta','H_{u} \ k',30,[],[],[]);
    ...
    SDOF_PlotFRF(r,@SDOF_FRF_Visc_mul_k,zeta_vec,'$r$','\zeta','H_{u} \ k',DispMagLines,r_peaks,abs(H_mag_peaks),[]);
...
SDOF_PlotFRF(r,@(r,zeta) (2*i*zeta*r+1)./(1-r.*r+2*i*zeta*r),zeta_vec,'$r$','\zeta','H_{u}',DispMagLines,r_peaks,abs(H_mag_peaks),sqrt(2));
...
SDOF_PlotFRF(r,@(r,zeta) r.*r./(1-r.*r+2*i*zeta*r),zeta_vec,'$r$','\zeta','H_{u}',DispMagLines,r_peaks,abs(H_mag_peaks),[]);
...
%Hysteretic SDOF
SDOF_PlotFRF(r,@SDOF_FRF_Hyst_mul_k,zeta_vec,'$r$','\eta','H_{u} \ k',DispMagLines,[],[],[]);
