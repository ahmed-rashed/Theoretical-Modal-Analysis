...
[EigVectors_Normalized, EigValues_mat]=MDOF_Visc_Eig(M, CC, K, false);
...
%plotResponse_ForceVertically(t_row,h_cols.',h_cols_label_col)
...
for ii=1:length(w_F1)
    ...
    plotResponse_ForceVertically(t_row,x_mat(:,1:N).',x_mat_label_col,f.',{'f_1'},['$w_{0}_1=',num2str(w_F1(ii)/w_d_r_vec(1)),'\ \omega_{\textrm{d},1}$'])
end
...
for ii=1:length(w_F1)
    ...
    plotResponse_ForceVertically(t_row,x_mat(:,1:N).',x_mat_label_col,f.',{'f_2'},['$w_{0}_2=',num2str(w_F1(ii)/w_d_r_vec(2)),'\ \omega_{\textrm{d},2}$'])
end